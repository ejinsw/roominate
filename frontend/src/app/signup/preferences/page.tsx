...
export default function PreferencesPage() {
    ...
    useEffect(() => {
        ...
        async function loadData() {
            ...
            try {
                ...
                const initialPreferences: { [key: string]: { option: string; importance: string } } = {};
    ...
    const handleAddHousing = () => {
        ...
        if (selectedHousingOptions.length >= MAX_HOUSING_PREFERENCES) {
            setHousingError(`Maximum ${MAX_HOUSING_PREFERENCES} housing preferences allowed`);
            return;
        }
    ...
    const handlePreferenceImportanceChange = (prefId: string, importance: string) => {
        setPreferences(prev => ({
            ...prev,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const formattedPreferences = Object.entries(preferences).map(([prefId, value]) => ({
                id: prefId,
                option: value.option,
        ...
        <div className="flex flex-col gap-8 justify-center items-center min-h-screen py-10 bg-gradient-to-b from-white to-[#E6F3FF]">
            ...
            >
                ...
                <div className="space-y-6">
                    <h3 className="text-lg font-medium text-[#2774AE] border-b pb-2">Personal Information</h3>
                    ...
                    <div className="w-full">
                        <div className="flex justify-between">
                            ...
                            <span className={`text-xs ${bio.length >= MAX_BIO_LENGTH * 0.9 ? 'text-red-500' : 'text-gray-500'}`}>
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between">
                            ...
                            <span className={`text-xs ${interests.length >= MAX_INTERESTS * 0.8 ? 'text-red-500' : 'text-gray-500'}`}>
                                {interests.length}/{MAX_INTERESTS}
                            </span>
                        </div>
                        <div className="flex flex-wrap gap-2 mb-2 min-h-[40px]">
                            {interests.map(interest => (
                                <div key={interest} className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full flex items-center">
                                    <span>{interest}</span>
                                    <button
                                        type="button"
                        ...
                        <div className="flex">
                            ...
                            <button
                                ...
                                disabled={interests.length >= MAX_INTERESTS}
                                className={`${interests.length >= MAX_INTERESTS ? 'bg-gray-400' : 'bg-[#2774AE] hover:bg-[#407ead]'} text-white px-4 py-2 rounded-r-lg transition-colors`}
                            >
                                Add
                            </button>
                        </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex">
                            ...
                            <button
                                ...
                                disabled={!selectedHousing || selectedHousingOptions.length >= MAX_HOUSING_PREFERENCES}
                                className={`${!selectedHousing || selectedHousingOptions.length >= MAX_HOUSING_PREFERENCES ? 'bg-gray-400' : 'bg-[#2774AE] hover:bg-[#407ead]'} text-white px-4 py-2 rounded-r-lg transition-colors`}
                            >
                                Add
                            </button>
                        </div>
                ...
                <div className="space-y-6 mt-8">
                    <h3 className="text-lg font-medium text-[#2774AE] border-b pb-2">Roommate Preferences</h3>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => handlePreferenceOptionChange(pref.id, e.target.value)}
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => handlePreferenceImportanceChange(pref.id, e.target.value)}
                                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#2774AE]"
                                    >
