...
export const getGroups = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      //add filter options
      if (req.query.id) {
        filter.id = { equals: parseInt(req.query.id.toString()) };
      }
      if (req.query.description) {
        filter.description = {
          contains: req.query.description.toString(),
          mode: "insensitive",
        };
      }
      if (req.query.numRoomates) {
        filter.numRoomates = {
          equals: parseInt(req.query.numRoomates.toString()),
        };
      }
      if (req.query.openToJoin) {
        filter.openToJoin = {
          equals: req.query.openToJoin.toString() === "true",
        };
      }
